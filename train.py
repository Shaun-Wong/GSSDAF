...
parser.add_argument('--num_param', default=5, type=int)
...
if args.model_name is None:
    if 'genia' in args.dataset_name:
        args.model_name = 'data/biobert-v1.1'
    elif args.dataset_name in ('ace2004', 'ace2005'):
        args.model_name = 'data/bert-large-uncased'
    else:
        args.model_name = 'data/bert-large-uncased'
...
def get_data(dataset_name, model_name):
    ...
    if dataset_name == 'ace2004':
        ...
        paths = 'preprocess/outputs/genia1'
    elif dataset_name == 'conll03':
        paths = 'preprocess/outputs/conll03'
    elif dataset_name == 'nne':
        paths = 'preprocess/outputs/NNE'
    else:
...
model = CNNNer(model_name, num_ner_tag=matrix_segs['ent'], cnn_dim=args.cnn_dim, biaffine_size=args.biaffine_size,
                ...
                kernel_size=kernel_size, n_head=n_head, cnn_depth=args.cnn_depth, num_param=args.num_param)
...
optimizer = torch.optim.AdamW([{'params': non_ln_params, 'lr': 7e-6, 'weight_decay': weight_decay},
                               {'params': ln_params, 'lr': 7e-6, 'weight_decay': 0},
...
def model_save(folder):
    torch.save(trainer.model.state_dict(), f'{dataset_name}_best_model.pt')
   
callbacks.append(CheckpointCallback(
    monitor='f#f#test',
    topk=1,
    model_save_fn=model_save
))
